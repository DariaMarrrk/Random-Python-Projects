    ...
    while not Placed and index != -1:
        if item < DataArray[index]:
            if index == StartPointer: #then we're adding to the beginning since the new data is smaller than the original smallest (on StartPointer)
                ...
                PointerArray.append(index)
                EndPointer += 1
                Placed = True
        else:
            previous_index = index
            index = PointerArray[previous_index]
...
global PointerArray
PointerArray = [] #stores pointers that point at the index in DataArray on which the next item in LL is located
global StartPointer
